...
export default function Todolist() {
    ...
    const changeIsCom = (index) => {
        functask(
            task.map((item) => {
                if (item.idnum === index) {

                    return { ...item, isCom: !item.isCom }
                }
                return item
            })
        )
    ...
    const alltask = task.map(({ name, idnum, isCom }) => {
        return (
            <div className={`${isCom ? "taskcon green" : "taskcon"}`}>
                ...
                <div className="btndiv">
                    ...
                    <div className="taskicon"><i class="fas fa-check" onClick={() => changeIsCom(idnum)}></i></div>
                </div>
            </div>
        )
    })
